...
final class TaskGenerationProcessorImpl implements TaskGenerationProcessor {
    ...
    @Override
    public void processGeneration(TaskGenerationRequestedEvent event) {
        var generationId = event.generationId();
        try {
            ...
            var result = checkSimilarityAndGenerate(generationId, generation.getPrompt());
            generation = getGenerationEntity(generationId);
            if (!generation.isInProgress()) {
                return;
            }
    ...
    @Override
    public void handleDeadLetter(TaskGenerationRequestedEvent event, Exception failure) {
        markFailed(event.generationId(),
                "task generation retries exhausted: "
                        + Objects.toString(failure.getMessage(), "task generation failed"));
        log.error("Task generation moved to DLT, generationId={}", event.generationId(), failure);
    }

    @Override
    public void markFailed(UUID generationId, String message) {
        taskGenerationRepository.findById(generationId).ifPresent(generation -> {
            if (generation.isInProgress()) {
